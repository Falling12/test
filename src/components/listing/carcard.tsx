...
export default async function CarCard({ car }: CarCardProps) {
    return (
        <Link href={`/details/${car.id}`} className="relative w-full aspect-[378/339] group hover:translate-y-[-5px] transition-all duration-300">
            <Image
            ...
            <div className='absolute inset-0 w-full h-full flex flex-col justify-between rounded-lg overflow-hidden'>
                ...
                <div className='flex flex-col gap-2 py-6 px-4' style={{
                    ...
                    <p className='text-white text-xl font-bold'>Havi {new Intl.NumberFormat('hu-HU', { style: 'currency', currency: 'HUF', minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(car.packages_prices?.subscription?.subscription_price_per_month ?? 0)} -tól</p>
                </div>
            </div>
